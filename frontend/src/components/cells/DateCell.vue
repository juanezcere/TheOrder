  ...
  computed: {
    show() {
      if (this.original != null) {
        if (this.original == '0000-00-00 00:00:00') return '';
        const date = new Date(this.original).toISOString();
        const original = date.slice(0, 10).toString();
        return original;
...
<template>
  <div>
    <input type="date" :placeholder="placeholder" :id="id + index" :name="name" :value="show"
